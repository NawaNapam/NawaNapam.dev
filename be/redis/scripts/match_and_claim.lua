...
-- validate candidate; if invalid, put them back and report
...
if (now - lastSeen) > stale then
  -- stale: put them back so they can be updated by heartbeat/auth later
  redis.call("SADD", "available", candidate)
  return ret({ ok = false, err = "STALE_PEER" })
end
...
-- remove both from pools (safe if requester was present)
